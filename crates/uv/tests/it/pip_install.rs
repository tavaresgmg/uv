...
fn abi_compatibility_on_freethreaded_python() {
    ...
    // Test 1: abi3 wheel should fail with a helpful error
    ...
    hint: The wheel is compatible with `abi3`, but you're using free-threaded CPython 3.14 (`cp314t`)
    ");

    // Test 2: cp314 (same version, GIL-enabled) wheel should fail with a helpful error
    let wheel_path = context
        .workspace_root
    ...
    hint: The wheel is compatible with CPython 3.14 (`cp314`) with the GIL enabled, but you're using free-threaded CPython 3.14 (`cp314t`)
    ");

    // Test 3: Wheel with both cp314t (compatible) and abi3 (incompatible) should succeed
    let wheel_path = context
        .workspace_root
